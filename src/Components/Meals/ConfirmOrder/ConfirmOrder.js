    ...
    <div className='focus'>
      ...
      <div key={itemIndex} >
        {quantities[itemIndex]?(<div>
          <h4 >{item.name} </h4>
           <d className='price'>${(item.price)}</d>    <span className="numberItem">x {quantities[itemIndex]} </span> <button className='AddButton' onClick={()=>Add(item.id)}>+</button> {quantities[itemIndex]?(<button className='MinseButton' onClick={()=>Minse(item.id)}>-</button>
          ):""} 
